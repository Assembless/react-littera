...
const Trans: React.FC<TransProps> = ({ children, values = {}, components = {} }) => {
   ...
   */
  const interpolate = (text: string): React.ReactNode => {
    let result = text;
  ...
  const renderChild = (child: React.ReactNode): React.ReactNode => {
    if (typeof child === "string") {
      ...
      const Component = typeof type === "string" ? components[type] : type;
      if (Component) {
        // Remove opening and closing tags if the component is parsed
  ...
  const renderedChildren = React.Children.map(children, renderChild);

  console.log("Rendered Children =>", renderedChildren);
